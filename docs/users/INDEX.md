# 用户文档

欢迎来到 PRISM-Gateway 用户文档！本目录包含面向终端用户的所有文档。

## 快速开始

| 文档 | 描述 |
|------|------|
| [getting-started.md](getting-started.md) | 快速入门指南 |
| [installation.md](installation.md) | 安装说明 |
| [quick-start.md](quick-start.md) | 快速开始使用 |
| [user-guide.md](user-guide.md) | 用户使用指南 |

## 功能文档

| 文档 | 描述 |
|------|------|
| [gateway-check.md](gateway-check.md) | Gateway 检查功能 |
| [retrospective.md](retrospective.md) | 复盘功能使用 |
| [analytics.md](analytics.md) | 分析功能使用 |

## 配置文档

| 文档 | 描述 |
|------|------|
| [configuration.md](configuration.md) | 系统配置说明 |
| [environment-variables.md](environment-variables.md) | 环境变量参考 |

## 常见问题

| 文档 | 描述 |
|------|------|
| [faq.md](faq.md) | 常见问题解答 |
| [troubleshooting.md](troubleshooting.md) | 故障排查指南 |

---

**相关链接：**
- [开发者文档](../developers/)
- [运维文档](../operators/)
- [主文档](../)
